var imageUploader = {
	...
	init : function() {
		alert('CC');
		document.getElementById("vProfileImageModalSrc").onclick = this.uploadImage
				.bind(this);
		/*document.getElementById("uploadCanvas").onclick = this.uploadCanvasAsImage
				.bind(this);*/
		...
		document.getElementById("file_1").onchange = function(event) {
			...
			/*document.getElementById("fileInfo").innerHTML = "File Name: "
					+ this.imageFile.name + "<br> Image type: "
					+ this.imageFile.type;*/
	...
	displayImage : function(img) {
		document.getElementById("vProfileImageModalSrc").src = img.src;
	},
	...
	/*canvasToBlob : function(canvas, type) {
		...
		var byteString = atob(canvas.toDataURL().split(",")[1]), ab = new ArrayBuffer(
				byteString.length), ia = new Uint8Array(ab), i;
	...
	/*uploadCanvasAsImage : function() {
		...
		var blob = this
				.canvasToBlob(this.canvasCtx.canvas, this.imageFile.type);
