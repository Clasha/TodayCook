...
public class RecipeSearchDateCommand implements RecipeCommand{
	//최근날짜순으로 레시피 리스트를 정렬하는 command

	@Override
	public void recipe(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		HttpSession session = request.getSession(); //세션 생성
		System.out.println("RecipeSearch <Date> Command 진입");
		Vector<RecipeListVo_main> list = new Vector<RecipeListVo_main>();
		RecipeDAO2 dao = RecipeDAO2.getInstance(); //DB접속
		list = dao.getDateList(); //최근날짜순으로 정렬된 리스트를 받는다
		System.out.println("list 발송!");
		request.setAttribute("Recipelist", list); //최근날짜순으로 정렬된 리스트를 발송한다
